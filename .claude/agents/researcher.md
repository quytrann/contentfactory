...
- **Agent Reach** (installed): to read facts that live on a real video/page rather than search snippets, call the upstream tools per `web-research` SKILL (`.\areach.ps1`, or `yt-dlp` directly). Main use = "watch" a YouTube video to mine topics / pull its transcript for script-gen; subtitles via `yt-dlp --write-auto-sub`, no-subtitle fallback = the project's local faster-whisper (not `agent-reach transcribe`, which needs an unset paid key). Never run `agent-reach configure`.
